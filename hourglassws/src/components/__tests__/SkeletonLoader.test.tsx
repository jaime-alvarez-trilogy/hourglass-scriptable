...
describe('SkeletonLoader — FR5: runtime render', () => {
  ...
  it('SC5.6 — custom width and height are passed to style prop', () => {
    ...
    // The rendered element should have width=120 and height=40 in its style
...
describe('SkeletonLoader — FR5: source file class strings and animation', () => {
  ...
  it('SC5.2 — source uses timingSmooth in withTiming call', () => {
    expect(source).toMatch(/withTiming\s*\([\s\S]{0,60}timingSmooth/);
  });
  ...
  it('SC5.3 — withRepeat uses reverse=true for ping-pong effect', () => {
    // withRepeat(withTiming(...), -1, true) — the third argument is reverse
    expect(source).toMatch(/withRepeat\s*\([\s\S]{0,120}true/);
  });
  ...
  it('SC5.8 — source does not use StyleSheet.create', () => {
    expect(source).not.toContain('StyleSheet.create');
  });

  it('SC5.8 — source does not import StyleSheet', () => {
